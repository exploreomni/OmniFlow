...
The helper creates **`OmniFlow deployment readiness`** on the exact current PR head SHA. One complete immutable changed-file inventory drives both routing and validation. Model changes require passing checks for every routed model; dbt-only changes require enabled dbt impact analysis with complete coverage and no findings. Unrelated changes can pass as not applicable only after trusted routing proves that no registered Omni or configured dbt paths changed. Unknown Omni paths, disabled/incomplete dbt checks, arbitrary skipped model/dbt reports, and mismatched report SHAs fail closed. Forks, drafts, stale checkout, and missing durable sync state are also rejected, including for unrelated PRs: adoption must bootstrap verified sync state first.

Before completing success the helper rereads head, base, and durable sync SHA. If any changed, dispatch a fresh run. A failed run restores the trusted policy's configured `pending_label` if it already removed it; if that restoration fails, the required check still fails and the label must be repaired manually. The helper never merges or sends a PR comment. API write failure cannot publish readiness success.

**Adopter configuration is mandatory:** require `OmniFlow deployment readiness` with the expected GitHub Actions source, require the PR to be up to date with its protected base, retain independent test/security checks and reviews, and prevent bypass. For dbt-enabled repos this fresh check is the merge gate for Omni validation; an old `pull_request_target` job may still report its earlier deployment hold and must not be treated as the current-head readiness evidence. The example does not automatically modify consumer branch protection. Dispatch readiness for **every same-repository PR** subject to this branch-wide required check, including additive model, dbt-only expansion, and documentation-only PRs. Keep configured dbt paths complete; unregistered source locations cannot be inferred automatically.
