...
| AI eval (opt-in, executes warehouse queries and incurs LLM spend) | Unit comparison, polling, scoring-grace, timeout, config, and public/restricted split tests | A real prompt set regression against a controlled non-production model fails the check, and no per-prompt cost or conversation detail appears in public artifacts |
