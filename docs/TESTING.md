...
| AI eval (opt-in, judge-scored rather than deterministic) | Unit comparison, polling, scoring-grace, timeout, config, and public/restricted split tests | A real prompt set regression against a controlled non-production model fails the check, and no per-prompt conversation detail appears in public artifacts |
