...
The optional, disabled-by-default AI eval check starts and polls documented `/v1/ai/eval/runs` operations for each configured prompt set, once against `main` and once against the branch. Omni AI answering those prompts can execute warehouse queries; OmniFlow does not control that, so the check is opt-in and documented in [AI Eval](AI_EVAL.md) rather than treated as core validation. Only bounded run-status GETs and non-retried run-creation POSTs are sent; full per-prompt run payloads (cost breakdown, conversation IDs, timing) are written to the restricted artifact path only, never `public/`. The public report carries aggregate accuracy and spend per prompt set plus a bounded, truncated list of regressed prompts.
