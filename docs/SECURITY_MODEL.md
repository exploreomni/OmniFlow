...
The optional, disabled-by-default AI eval check starts and polls documented `/v1/ai/eval/runs` operations for each configured prompt set, once against `main` and once against the branch. Only bounded run-status GETs and non-retried run-creation POSTs are sent; full per-prompt run payloads (conversation IDs, timing) are written to the restricted artifact path only, never `public/`. The public report carries aggregate accuracy per prompt set plus a bounded, truncated list of regressed prompts. See [AI Eval](AI_EVAL.md).
